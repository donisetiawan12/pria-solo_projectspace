...
import AuthModal from '../components/AuthModal'; // Path disesuaikan karena ada di dalam folder pages
...
export default function LandingPage({ onLogin, onExplore }) {
  ...
  const handleLoginSuccess = () => {
    setIsAuthOpen(false);
    onLogin(); // Picu fungsi sukses login di App.jsx
  };
    ...
    <div className="min-h-screen bg-[#0d1117] text-[#c9d1d9] relative overflow-hidden font-sans selection:bg-blue-500/30">
      ...
      <nav className="w-full max-w-7xl mx-auto px-6 py-4 flex justify-between items-center border-b border-[#21262d] relative z-10">
        ...
        <button 
          ...
          className="bg-[#0a66c2] hover:bg-[#004182] text-white text-xs font-bold px-5 py-2.5 rounded-lg transition-all duration-200 active:scale-95"
        >
          Masuk / Sign In
        </button>
      </nav>
      ...
      <main className="max-w-4xl mx-auto px-6 pt-24 pb-16 text-center relative z-10 flex flex-col items-center justify-center min-h-[calc(100vh-120px)]">
        <div className="inline-flex items-center bg-[#111827] border border-[#1d4ed8]/40 px-4 py-1.5 rounded-full text-[10px] uppercase tracking-widest font-mono text-blue-400 font-bold mb-8">
        ...
        <h1 className="text-4xl md:text-6xl font-extrabold text-white tracking-tight leading-[1.15] mb-6 max-w-3xl">
          ...
          <span className="relative inline-block text-transparent bg-clip-text bg-gradient-to-r from-blue-400 via-sky-400 to-indigo-400">
            Jaringan Profesional
          </span> Anda.
        </h1>

        <p className="text-sm md:text-base text-[#8b949e] max-w-2xl leading-relaxed mb-10 font-medium">
          ProjectSpace adalah wadah hibrida tempat mahasiswa memamerkan kode, skema basis data MySQL, ulasan tugas, hingga integrasi hardware IoT. Dapatkan saran konstruktif dan jalin kolaborasi lintas semester!
        </p>

        <div className="flex flex-col sm:flex-row gap-4 w-full justify-center items-center">
          {/* Tombol Jelajahi (Mode Tamu / Belum Login) */}
          <button 
            onClick={onExplore}
            className="w-full sm:w-auto bg-[#2ea44f] hover:bg-[#39d353] text-white font-bold text-sm px-8 py-3 rounded-xl transition-all duration-200 shadow-md active:scale-98"
          >
            Jelajahi Feed Karya
          </button>
          ...
          <button 
            ...
            className="w-full sm:w-auto bg-transparent border border-[#30363d] hover:border-[#8b949e] hover:bg-[#21262d] text-white font-bold text-sm px-8 py-3 rounded-xl transition-all duration-200 active:scale-98"
          >
            Daftarkan Akun Anda
          </button>
        </div>
